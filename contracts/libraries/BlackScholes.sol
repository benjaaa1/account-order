...
library BlackScholes {
    ...
    function pricesDeltaStdVega(
        BlackScholesInputs memory bsInput
    ) public pure returns (PricesDeltaStdVega memory) {
        uint tAnnualised = _annualise(bsInput.timeToExpirySec);
        uint spotPrecise = bsInput.spotDecimal.decimalToPreciseDecimal();
        ...
        (uint vegaPrecise, uint stdVegaPrecise) = _standardVega(
            d1,
            spotPrecise,
    ...
    function delta(
        BlackScholesInputs memory bsInput
    ) public pure returns (int callDeltaDecimal, int putDeltaDecimal) {
        uint tAnnualised = _annualise(bsInput.timeToExpirySec);
        uint spotPrecise = bsInput.spotDecimal.decimalToPreciseDecimal();
    ...
    function vega(
        BlackScholesInputs memory bsInput
    ) public pure returns (uint vegaDecimal) {
        uint tAnnualised = _annualise(bsInput.timeToExpirySec);
        uint spotPrecise = bsInput.spotDecimal.decimalToPreciseDecimal();
    ...
    function _d1d2(
        ...
        int log = FixedPointMathLib.lnPrecise(
    ...
    function _standardVega(
        ...
        uint normalisationFactor = _getVegaNormalisationFactorPrecise(
            timeToExpirySec
        );
    ...
    function _stdNormalCDF(int x) public pure returns (uint) {
        ...
        if (z <= 37 * PRECISE_UNIT) {
            uint e = FixedPointMathLib.expPrecise(
